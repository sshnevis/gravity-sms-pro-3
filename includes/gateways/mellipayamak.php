...
class GFHANNANSMS_Pro_MELLIPAYAMAK {
	...
	*/	
	...
	if ($action == "send"){
		...
		if(trim($key) == 'shared'){
			$arrayTostr = implode(";",$arraytext);
			$arr2=explode(";",$arrayTostr);
			$bodyid = intval(array_pop($arr2));
				...
				$send = $client->SendByBaseNumber($data)->SendByBaseNumberResult;
				}
	...
	if ($action == "credit") {
    				$client = new SoapClient("http://api.payamak-panel.com/post/send.asmx?wsdl");	
    				$credit = $client->GetCredit(array("username"=> $username ,"password"=>$password))->GetCreditResult;
    			}
		...
		if ($action == "send"){
		    if($send== -5){
		        return __('متن ارسالی با توجه به متغیرهای مشخص شده در متن پیشفرض همخوانی ندارد .', 'GF_SMS' );
		    }
		    elseif($send == -4){
		        __('کد متن ارسالی صحیح نمیباشدو یا تأیید نشده است .', 'GF_SMS' );
		    }
			elseif ( $send == 0 ) {
